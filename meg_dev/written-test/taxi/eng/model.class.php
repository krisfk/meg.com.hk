...
//		DB::$dbName = 'meghongk_db3';		
	...
	private function getP2Categories(){
		$categoies = DB::query("SELECT category, count(*) FROM taxi_question_p2_en GROUP BY category");
		return $categories;
	}
	
	private function part2Table($part){
		return 'taxi_p'.$part.'_question_en';
	}
	...
	public function randP2(){
		$questions = DB::query('select * from taxi_p2_question_en');
		foreach($questions as $q){
			...
			DB::update('taxi_p2_question_en', array(
			  'answer1' => $answers[0],
			  'answer2' => $answers[1],
			  'answer3' => $answers[2],
			  'answer4' => $answers[3],
